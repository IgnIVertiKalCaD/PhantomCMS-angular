...
import {FetchDescription, ServersState} from './store/servers.state';
...
@Component({
  ...
  standalone: true,
  imports: [MatProgressBarModule, MatChipsModule, NgForOf, NgClass],
})
export class ServersComponent implements OnInit{
  constructor(private readonly sanitizer: DomSanitizer, private store: Store) {}

  @Select(ServersState.getServersName) servers$: Observable<any>;
  ...
  ngOnInit() {
    ...
    this.store.dispatch(new FetchDescription());
